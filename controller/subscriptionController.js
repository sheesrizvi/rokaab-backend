...
const allSubscription = asyncHandler(async (req, res) => {
  ...
  if (subscription) {
    res.json({ subscription, pageCount });
  } else {
    console.log("error");
  }
});
...
const subsChange = asyncHandler(async (req, res) => {
  const { userId, grace, id, planId } = req.body;
  ...
  if (grace) {
    ...
    const newexpiry = new Date(
      subscription.expiry.getTime() + grace * 24 * 60 * 60 * 1000
    );
    ...
    user.subscriptionStatus = true;
    const renewedSubscription = await subscription.save();
    const renewedUser = await user.save();
    if (renewedSubscription) {
      res.status(201).json({
        renewedSubscription,
      });
